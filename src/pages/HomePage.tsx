...
export const HomePage: React.FC = () => {
  ...
  const {
    isLoading: brandNewModelsLoading,
    data: brandNewModels,
    fetchNextPage: fetchNextBrandNewModels,
  } = useInfiniteQuery({
    queryKey: ['brandNewModels'],
    queryFn: data => getBrandNewModels(data.pageParam, 10),
    getNextPageParam: (_, pages) => pages.length,
    initialPageParam: 0,
  });

  const {
    isLoading: hotPriceProductsLoading,
    data: hotPriceProducts,
    fetchNextPage: fetchNextPageHotPriceProducts,
  } = useInfiniteQuery({
    queryKey: ['hotPriceProducts'],
    queryFn: ({ pageParam }) => getHotPriceProducts(pageParam, 10),
    getNextPageParam: (_, pages) => pages.length,
    initialPageParam: 0,
  });
  ...
  return (
    <main className="flex flex-col items-center gap-16 pb-16 pt-6">
      ...
      {brandNewModelsLoading ? (
        ...
        !!brandNewModels?.pages.length && (
          <ProductsSlider
            className="content-padding"
            discount={false}
            title="Brand new models"
            swiperProps={{
              onReachEnd: () => fetchNextBrandNewModels(),
            }}
            slides={brandNewModels.pages.flat()}
          />
        )
      )}
      ...
      {hotPriceProductsLoading ? (
        ...
        !!hotPriceProducts?.pages.length && (
          <ProductsSlider
            className="content-padding"
            title="Hot prices"
            swiperProps={{
              onReachEnd: () => fetchNextPageHotPriceProducts(),
            }}
            slides={hotPriceProducts.pages.flat()}
          />
        )
      )}
    </main>
